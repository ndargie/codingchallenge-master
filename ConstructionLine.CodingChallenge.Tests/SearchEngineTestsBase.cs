...
namespace ConstructionLine.CodingChallenge.Tests
{
    ...
    protected static void AssertSizeCountsShouldBeInSearch(List<Shirt> shirts, SearchOptions searchOptions, List<SizeCount> sizeCounts)
    {
      Assert.That(sizeCounts, Is.Not.Null);
      ...
      foreach (var size in Size.All)
      {
        ...
        var expectedSizeCount = searchOptions.Sizes.Any(x => x.Id == size.Id) ? shirts
            .Count(s => s.Size.Id == size.Id
                        && (!searchOptions.Colors.Any() || searchOptions.Colors.Select(c => c.Id).Contains(s.Color.Id))) : 0;
    ...
    protected static void AssertColorShouldBeInSearchCounts(List<Shirt> shirts, SearchOptions searchOptions, List<ColorCount> colorCounts)
    {
      Assert.That(colorCounts, Is.Not.Null);
      ...
      foreach (var color in Color.All)
      {
        ...
        var expectedColorCount = searchOptions.Colors.Any(x => x.Id == color.Id) ? shirts
            .Count(c => c.Color.Id == color.Id
                        && (!searchOptions.Sizes.Any() || searchOptions.Sizes.Select(s => s.Id).Contains(c.Size.Id))) : 0;
